...
namespace EasyLayout {
	...
	public class EasyLayout : UnityEngine.UI.LayoutGroup  {
		...
		/// The filter.
		/// </summary>
		public Func<IEnumerable<GameObject>,IEnumerable<GameObject>> Filter = x => x;
		...
		public void UpdateLayout()
		{
			...
			start_position.x += Margin.x * ((1 - anchor_position.x) * 2 - 1);
			start_position.y += Margin.y * ((1 - anchor_position.y) * 2 - 1);
		...
		List<List<RectTransform>> GroupUIElements()
		{
			...
			base_length -= 2 * ((Stacking==Stackings.Horizontal) ? Margin.x : Margin.y);
