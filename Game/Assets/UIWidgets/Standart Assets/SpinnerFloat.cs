...
namespace UIWidgets {
	...
	public class SpinnerFloat : InputField, IPointerDownHandler {
		...
		void ValueChange(string value)
		{
			...
			_value = float.Parse(value, System.Globalization.CultureInfo.InvariantCulture);
		...
		char IntValidate(string validateText, int charIndex, char addedChar)
		{
			...
			if (number.Length >= min_parse_length)
			{
				...
				float new_value;
				if ((!float.TryParse(number, style, System.Globalization.CultureInfo.InvariantCulture, out new_value)))
				{
					return (char)0;
				}
		...
		#if UNITY_EDITOR
		[UnityEditor.MenuItem("GameObject/UI/SpinnerFloat")]
		static void CreateObject()
		{
			Utilites.CreateObject("Assets/UIWidgets/Prefabs/SpinnerFloat.prefab");
		}
		#endif
	}
}